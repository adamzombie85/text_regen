...
2. **嚴格長度控制 (最重要任務)**：
   - 您的改寫後文本「絕對不可超過」{word_count} 字。
   - 這是一個硬性限制，如果輸出超過 {word_count} 字，您的任務即視為失敗。
   - 請勇於刪除非核心的細節、形容詞、對話或背景描述，只保留主幹，以確保達成字數目標。
   - 寧可字數稍微少於 {word_count}，也絕對不能多於它。
3. **難度控制**：儘量使用基礎詞彙（目標是讓初學者能讀懂）。
4. **格式要求**：只輸出改寫後的文本內容，不要有任何解釋、開場白或結尾。不要在回覆中計算字數。
...
- **用字與識字難度限制**：
  * **用字標準**：優先使用『教育部 700 個台語推薦用字』。
  * **重要修正**：表示「最」的程度副詞一律使用『上 (siōng)』，**絕對禁止使用『尚』**（例：上好、上基本）。
